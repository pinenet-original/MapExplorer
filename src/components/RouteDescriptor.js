...
export const RouteDescriptor = ({
  ...
  const checkDistance = (idx, routeItem) => {
    if (idx === 0) {
      ...
      return calculateDistance(
        route.data[idx - 1].latitude,
        route.data[idx - 1].longitude,
        route.data[idx].latitude,
        route.data[idx].longitude
      );
    }
  };
  ...
  return (
    <div className="text-center px-4">
      <h1 className="text-4xl mb-5">{route.routeTitle}</h1>
      <ul className="mb-5 max-w-xs text-left mx-auto">
        {route?.data.map((routeItem, idx) => {
          return (
            ...
            >
              <span>
                {`${idx + 1} ) ${routeItem.markerName} - ${checkDistance(
                  idx,
                  routeItem
                )}m`}
              </span>
