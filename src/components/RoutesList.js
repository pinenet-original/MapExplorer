    ...
    <div className={styles.routeListSelector}>
        {listOfRoutes.map((route, idx) => {
          return (
            ...
            <button key={route.routeTitle} onClick={()=> {routeSelectManager(idx)}} className="bg-blue-500 hover:bg-blue-700 text-white font-bold py-2 px-4 rounded-full">НАЧНИ ИГРУ</button>
          )
        })}
    </div>
  );
